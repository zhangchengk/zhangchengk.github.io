...
10. <font color=green>【推荐】</font>表的命名最好是遵循“业务名称_表的作用”。
:::tip 正例
...
13. <font color=green>【推荐】</font>字段允许适当冗余，以提高查询性能，但必须考虑数据一致。冗余字段应遵循：
- 1） 不是频繁修改的字段。
- 2） 不是唯一索引的字段。 
...
:::
15. <font color=orange>【参考】</font>合适的字符存储长度，不但节约数据库表空间、节约索引存储，更重要的是提升检索速度。
:::tip 正例
无符号值可以避免误存负数，且扩大了表示范围。
:::
![](https://gitee.com/zhangchengk/img/raw/master/img//Users/zhangcheng/vscodeProjects/image/阿里Java开发手册/mysql.png)