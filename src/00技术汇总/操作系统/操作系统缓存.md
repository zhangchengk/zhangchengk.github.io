...
![L1, L2 and L3 cache in a four core processor (credit)](./img/cpucache.png)
...
![Row wise access efficiently uses the cache organization](./img/cpucacheline.png)
...
![Column wise access is inefficient as it results in swapping of cache lines](./img/cpucacheline2.png)

## 优先选择按值存储的数组和向量

缓存组织还可以更好地用于按值存储的数据结构。考虑一下向量指针的缓存行分配：`vector<Entity*>`。如果从堆中分配了单独的Entity对象，则很可能所有的Entity对象都在内存中相距很远。这意味着访问每个实体将导致加载完整的缓存行。

![Cache line usage in a vector of pointers](./img/vector.png)

将上述情况与按值存储实体的向量进行比较：`vector<Entity>`。在这种情况下，实体对象被组织在连续的内存中。这样可以最大程度地减少缓存行的负载。

![Cache line usage in a vector that stored data by value](./img/vector2.png)
...
![Cache behavior when the validity flag is stored inside an array](./img/cpucacheline3.png)
...
![Cache behavior when the validity flags are stored in a separate descriptor](./img/cpucacheline4.png)
...
![Example of false sharing due to two cores sharing a single cache line](./img/cpucacheline5.png)
