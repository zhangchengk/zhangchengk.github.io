...
6. <font color=green>【推荐】</font>不要在视图模板中加入任何复杂的逻辑。
:::warning 说明
根据 MVC 理论，视图的职责是展示，不要抢模型和控制器的活。
:::
7. <font color=green>【推荐】</font>任何数据结构的构造或初始化，都应指定大小，避免数据结构无限增长吃光内存。
8. <font color=green>【推荐】</font>及时清理不再使用的代码段或配置信息。
:::warning 说明
