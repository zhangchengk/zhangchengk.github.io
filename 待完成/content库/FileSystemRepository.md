...
public class FileSystemRepository implements ContentRepository {
    ...
    // 可配置NiFiProperties.MAX_APPENDABLE_CLAIM_SIZE属性的上限为100 MB，以防止不必要的大型资源声明文件
    ...
    // Queue for claims that are kept open for writing. Ideally, this will be at least as large as the number of threads that will be updating the repository simultaneously but we don't want to get too large because it will hold open up to this many FileOutputStreams.
    // The queue is used to determine which claim to write to and then the corresponding Map can be used to obtainthe OutputStream that we can use for writing to the claim.
